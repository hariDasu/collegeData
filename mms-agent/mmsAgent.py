...
# Python
import threading, urllib, urllib2, socket, zlib, time, sets, re, gc, subprocess, os
...
class MmsAgent( object ):
    ...
    def __init__( self, settings, agentVersion, pythonVersion, pymongoVersion, pymongoHasC, agentHostname, logger, sessionKey ):
        ...
        self.collectionIntervalFactor = 1
        ...
        self.disableLocksAndRecordStatsDataCollection = settings.disableLocksAndRecordStatsDataCollection
    ...
    def getDbConnection( self, hostKey ):
        ...
        args = { "slave_okay" : True,
                 "document_class" : bson.son.SON,
                 "ssl" :  self.settings.useSslForAllConnections or ('ssl' in hostDef and hostDef['ssl'])
               }

        if args["ssl"] and self.settings.sslRequireValidServerCertificates:
            if not HAS_SSL:
                raise Exception("SSL required for connection but Python SSL module not found.")
            args["ssl_cert_reqs"] = ssl.CERT_REQUIRED
            args["ssl_ca_certs"] = self._get_trusted_certs_path()

        conn = pymongo.Connection( hostDef['mongoUri'] , **args )

        if self.settings.krb5Principal and self.settings.krb5Keytab:
            self._authenticate(conn)

        return conn

    def _get_trusted_certs_path(self):
        if self.settings.sslTrustedServerCertificates is None:
            raise Exception("SSL required for connection but `sslTrustedServerCertificates` setting is not defined.")

        if not os.path.isfile(self.settings.sslTrustedServerCertificates):
            raise Exception("SSL required for connection but `sslTrustedServerCertificates` file can not be found.")

        return self.settings.sslTrustedServerCertificates

    def _authenticate( self, conn ):
        try:
            conn.admin.authenticate(self.settings.krb5Principal, source='$external', mechanism='GSSAPI')
        except Exception:
            self.logger.warning('GSSAPI authentication failed. Trying to call kinit.')
            self._callKinit()
            conn.admin.authenticate(self.settings.krb5Principal, source='$external', mechanism='GSSAPI')

    def _callKinit( self ):
        try:
            subprocess.check_call(['/usr/bin/kinit', '-k', '-t', self.settings.krb5Keytab, self.settings.krb5Principal])
        except subprocess.CalledProcessError, e:
            self.logger.warning('Failed to run kinit: ' + str(e))
    ...
    def extractHostDef( self, host ):
        ...
        if hostKey not in self.serverHostDefs:
            ...
            hostDef['hostTypes'] = host['hostTypes']
        else:
            hostDefLast = { }
            hostDef = self.serverHostDefs[hostKey]
            ...
            hostDefLast['hostTypes'] = hostDef['hostTypes']
    ...
    def stopAndClearHost( self, hostKey ):
        ...
        self.logger.info( "Stopping monitoring for %s" % hostKey )
    ...
    def getSendDataInterval( self ):
        return max(1, self.collectionInterval / self.collectionIntervalFactor )

    def getCollectionInterval( self ):
        return max(1, ((self.collectionInterval / self.collectionIntervalFactor) / 2) - 1)

    def getConfInterval( self ):
        return max(1, self.confInterval / self.collectionIntervalFactor)
