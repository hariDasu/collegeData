...
blockingStatsAgentVersion = "1.6.8"

CONFIG_SVR_HOST_TYPE_ID = 3
...
class BlockingMongoStatsThread( threading.Thread ):
    ...
    def run( self ):
        ...
        self.logger.info( 'Starting blocking stats monitoring' )
        ...
        while not self.mmsAgent.done and self.running:
            try:
                time.sleep( self.mmsAgent.getCollectionInterval() )
                passes = passes + 1
                ...
                monitorConn = self.mmsAgent.closeDbConnection( self.hostKey, monitorConn )
                self.logger.exception( 'Problem collecting blocking data: ' + e.message )

        self.logger.info( 'Stopping blocking stats monitoring' )
    ...
    def _collectBlockingStats( self, passes, monitorConn ):
        ...
        try:
            ...
            if 'serverStatus' in stats:
                self.mmsAgent.setHostState(self.hostKey, 'mongoBlockingServerStatus', stats['serverStatus'])
                del stats['serverStatus']
    ...
    def _collectStats( self, passes, monitorConn ):
        ...
        hostTypes = self.mmsAgent.hostDefValue( self.hostKey, 'hostTypes' )

        isConfigSvr = (hostTypes is not None and CONFIG_SVR_HOST_TYPE_ID in hostTypes)

        isMongos = ( 'msg' in isMaster and isMaster['msg'] == 'isdbgrid' )

        # If this is a config server then pull the shard information
        try:
            if isConfigSvr:
                root['configShards'] = list( monitorConn.config.shards.find() )
        ...
        # For MongoDB 2.4+, exclude db-specific elements from serverStatus, if dbStats are disabled
        if self.mmsAgent.disableLocksAndRecordStatsDataCollection:
            root['serverStatus'] = monitorConn.admin.command( 'serverStatus', recordStats=False, locks=False)
        else:
                    ...
                    if passes % 20 == 0:
                        ...
                        if not self.mmsAgent.disableDbstats:
                            ...
                            if ( time.time() - startTime ) > 6:
                                ...
                                self.logger.warn( 'Collection of dbStats took longer than allowed threshold' )
                            else:
                                self.slowDbStats = False
