    ...
    def run( self ):
        ...
        while not self.mmsAgent.done and self.running:
            try:
                ...
                logConn = self.mmsAgent.closeDbConnection( self.hostKey, logConn )
                self.logger.warn( 'Problem collecting log data ' + e.message, exc_info=1 )
    ...
    def _processLogs( self, logConn ):
        ...
        try:
            ...
            if not self.mmsAgent.hasCommand( 'getLog', self.hostKey, logConn ):
                self.logger.warn( 'This version of MongoDB does not support the getLog command' )
                time.sleep( 60 )
                return False
            ...
            self.logger.warn( 'Could not collect log data, logs contained non-UTF-8 characters', exc_info=1 )
            return False
        except Exception, e:
            self.logger.warn( 'Problem collecting log data: ' + e.message, exc_info=1 )
            return False
